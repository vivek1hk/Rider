...
public class CarController : MonoBehaviour {
	...
	void Update () 
	{
		...
		{
			 accelKey = Input.GetKey (KeyCode.W);
			 brakeKey =  Input.GetKey (KeyCode.S);
		}
		...
		else if (accelKey)//pxsLeapInput.GetPitch()>10f)
		{
		...
		if (brakeKey)//pxsLeapInput.GetPitch()>10f)
		{
			if (drivetrain.slipRatio < 0.2f)
